...
7. In **Media**, use the image upload control inside the Pages, Testimonials, or Associated Banks editors. In development, the API stores uploaded files in `public/uploads`; when `BLOB_READ_WRITE_TOKEN` is configured, it stores them in Vercel Blob and records the URL in Turso.
